...
export default defineEventHandler(async (event) => {
  ...
  try {
    // Get current print queue items
    const queueItems = await printQueueService.getQueue()
    
    // Get queue status for additional metadata
    ...
    return {
      ...
      meta: {
        ...
        readyToPrint: queueStatus.readyToPrint,
        requiresWarning: queueStatus.requiresWarning,
        lastUpdated: new Date().toISOString()
      }
    }

  } catch (error) {
    console.error('Error retrieving print queue:', error)
    
    throw createError({
      statusCode: 500,
      statusMessage: 'Failed to retrieve print queue'
    })
  }
})